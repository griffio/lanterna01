...
sealed class Tile(val char: Char, val color: ANSI) {
    class Floor() : Tile('\u00B7', ANSI.YELLOW)
    class Wall() : Tile('\u2593', ANSI.CYAN)
    class Bounds() : Tile('x', ANSI.BLACK)
}

class WorldBuilder(private val height: Int, private val width: Int) {

    var worldTiles: Table<Int, Int, Tile> = randomWorld(ArrayTable.create<Int, Int, Tile>(0 until height, 0 until width))
...
class WorldWriter(private val screen: Screen) {
    ...
    fun drawWorld() {
        ...
        for (tile in tiles) {
            text.setCharacter(tile.columnKey!!, tile.rowKey!!, TextCharacter(tile.value!!.char, tile.value!!.color, ANSI.DEFAULT))
        }
    }
}
...
fun draw(writer: DefaultWriter) {
    writer.centerText("Press any key to quit...", SGR.BLINK)
